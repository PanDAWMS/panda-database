...
--------------------------------------------------------
  ...
  INSERT INTO "ATLAS_PANDA"."PANDADB_VERSION" VALUES ('PANDABIGMON', 0, 0, 15);
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "ATLAS_PANDA"."FILESTABLE4_ROW_ID_SEQ"  MINVALUE 1 MAXVALUE 999999999999999999999999 INCREMENT BY 1 START WITH 29609319414 CACHE 20 NOORDER  NOCYCLE ;

 --------------------------------------------------------
--  DDL for Sequence GROUP_JOBID_SEQ
--------------------------------------------------------

   CREATE SEQUENCE  "ATLAS_PANDA"."GROUP_JOBID_SEQ"  MINVALUE 1 MAXVALUE 999999999999999999999999999 INCREMENT BY 1 START WITH 691526 NOCACHE  NOORDER  NOCYCLE ;

--------------------------------------------------------
--  DDL for Sequence HARVESTER_COMMAND_ID_SEQ
--------------------------------------------------------

   CREATE SEQUENCE  "ATLAS_PANDA"."HARVESTER_COMMAND_ID_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 21835001 NOCACHE  NOORDER  NOCYCLE ;

--------------------------------------------------------
--  DDL for Sequence JEDI_DATASETS_ID_SEQ
--------------------------------------------------------

   CREATE SEQUENCE  "ATLAS_PANDA"."JEDI_DATASETS_ID_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 301251834 NOCACHE  NOORDER  NOCYCLE ;

--------------------------------------------------------
--  DDL for Sequence JEDI_DATASET_CONT_FILEID_SEQ
--------------------------------------------------------

   CREATE SEQUENCE  "ATLAS_PANDA"."JEDI_DATASET_CONT_FILEID_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 20468197072 CACHE 100 NOORDER  NOCYCLE ;

--------------------------------------------------------
--  DDL for Sequence JEDI_OUTPUT_TEMPLATE_ID_SEQ
--------------------------------------------------------

   CREATE SEQUENCE  "ATLAS_PANDA"."JEDI_OUTPUT_TEMPLATE_ID_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 55776046 NOCACHE  NOORDER  NOCYCLE ;
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "ATLAS_PANDA"."JOBSDEFINED4_PANDAID_SEQ"  MINVALUE 1002000000 MAXVALUE 999999999999999999999999999 INCREMENT BY 1 START WITH 4674370997 NOCACHE  NOORDER  NOCYCLE ;
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "ATLAS_PANDA"."RETRYERRORS_ID_SEQ"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 43 NOCACHE  NOORDER  NOCYCLE ;
...
--------------------------------------------------------
  ...
  CREATE TABLE "ATLAS_PANDA"."JEDI_DATASETS" 
   (	"JEDITASKID" NUMBER(11,0), 
	...
	"NEVENTS" NUMBER(11,0), 
	"NEVENTSTOBEUSED" NUMBER(11,0), 
	"NEVENTSUSED" NUMBER(11,0), 
	"LOCKEDBY" VARCHAR2(40 BYTE), 
	"LOCKEDTIME" DATE, 
...
--------------------------------------------------------
--  DDL for Procedure VERIF_DROP_COPIEDPANDAPART_V2
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ATLAS_PANDA"."VERIF_DROP_COPIEDPANDAPART_V2"             (DAYS_OFFSET NUMBER default 2)
AUTHID CURRENT_USER
AS
...
SELECT COUNT(*) INTO num_part
FROM ATLAS_PANDA.tablepart4copying
WHERE
TRUNC(copying_done_on) =
(
SELECT MIN(TRUNC(copying_done_on)) FROM ATLAS_PANDA.tablepart4copying WHERE copied_to_arch = 'Y' AND deleted_on is NULL
)
AND deleted_on is NULL
AND (TRUNC(sysdate) - TRUNC(copying_done_on)) >= DAYS_OFFSET;
...
IF ( coll_tables.COUNT > 0 AND coll_tables.COUNT = coll_parts.COUNT ) THEN
	...
	FOR j IN 1 .. coll_tables.COUNT LOOP
		...
		IF coll_tables(j) = 'JOBSARCHIVED4' THEN
			...
			from ATLAS_PANDA.JOBSARCHIVED4 PARTITION(' || coll_parts(j) || ') tab
			WHERE NOT EXISTS (SELECT /*+ FULL(subt) CARDINALITY(subt 2000000) */ 1 FROM ATLAS_PANDAARCH.JOBSARCHIVED subt
			where PANDAID= tab.PANDAID
			AND MODIFICATIONTIME >= (TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'') - 1)  AND MODIFICATIONTIME <
			TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'')) ' ;
			...
			IF (jobs_diff = 0)  THEN
				UPDATE atlas_panda.tablepart4copying SET DATA_VERIF_PASSED = 'YES', data_verified_on = SYSDATE WHERE table_name = coll_tables(j) AND partition_name = coll_parts(j);
			ELSE
				UPDATE atlas_panda.tablepart4copying SET DATA_VERIF_PASSED = 'NO', data_verified_on = SYSDATE WHERE table_name = coll_tables(j) AND partition_name = coll_parts(j);							commit;
				raise_application_error(-20101, 'Not all rows copied from table JOBSARCHIVED4');
				RETURN;
			END IF;
		END IF;
		...
		IF coll_tables(j) = 'JOBPARAMSTABLE' THEN
			...
			stmt :=	'SELECT
				...
				*/
				COUNT(pandaid)
				FROM ATLAS_PANDA.JOBPARAMSTABLE PARTITION(' || coll_parts(j) || ') tab
			WHERE NOT EXISTS (SELECT 1 FROM ATLAS_PANDAARCH.JOBPARAMSTABLE_ARCH subt
			where PANDAID= tab.PANDAID
			AND MODIFICATIONTIME >= (TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'') - 1)  AND MODIFICATIONTIME <
			TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'')) ' ;
		...
		IF coll_tables(j) = 'METATABLE' THEN
			...
			stmt :=	'SELECT
					...
					*/
				COUNT(pandaid)
				FROM ATLAS_PANDA.METATABLE PARTITION(' || coll_parts(j) || ') tab
			WHERE NOT EXISTS (SELECT 1 FROM ATLAS_PANDAARCH.METATABLE_ARCH subt
			where PANDAID= tab.PANDAID
			AND MODIFICATIONTIME >= (TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'') - 1)  AND MODIFICATIONTIME <
			TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'')) ' ;
		...
		IF coll_tables(j) = 'FILESTABLE4' THEN
			...
			stmt:='SELECT
				 ...
				 */
				COUNT(row_id)
				FROM ATLAS_PANDA.filestable4 PARTITION('|| coll_parts(j) ||') tab
			WHERE NOT EXISTS (SELECT 1 FROM ATLAS_PANDAARCH.FILESTABLE_ARCH subt
			WHERE ROW_ID = tab.ROW_ID AND MODIFICATIONTIME >= (TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'') - 1)
			AND MODIFICATIONTIME < TO_DATE('''|| SUBSTR(coll_parts(j), -8) ||''', ''DDMMYYYY'')) ' ;
	...
	FOR j IN 1 .. coll_tables.COUNT LOOP
		...
		LOOP
		   BEGIN
			...
			-- add info into the logging table 'tablepart4copying'
			UPDATE atlas_panda.tablepart4copying SET deleted_on = SYSDATE WHERE table_name = coll_tables(j) AND partition_name = coll_parts(j);
			COMMIT;
		     	EXIT;
		   EXCEPTION
    			WHEN resource_busy THEN DBMS_LOCK.sleep(1);
...
--------------------------------------------------------
--  DDL for Procedure VERIF_DROP_COPIEDPANDAPART_V3
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ATLAS_PANDA"."VERIF_DROP_COPIEDPANDAPART_V3"                                                 (DAYS_OFFSET NUMBER default 2)
AUTHID CURRENT_USER
AS
...
ATLAS_PANDA.tablepart4copying
WHERE
table_name <> 'FILESTABLE4' 
AND 
TRUNC(copying_done_on) =
(
SELECT MIN(TRUNC(copying_done_on)) FROM ATLAS_PANDA.tablepart4copying WHERE table_name <> 'FILESTABLE4' AND copied_to_arch = 'Y' AND deleted_on is NULL
)
AND deleted_on is NULL
AND ( TRUNC(sysdate) - TRUNC(copying_done_on) ) >= DAYS_OFFSET
...
--------------------------------------------------------
  ...
  ALTER TABLE "ATLAS_PANDA"."JOBSWAITING4" ADD CONSTRAINT "JOBSWAITING4_WORKQUEUE_ID_FK" FOREIGN KEY ("WORKQUEUE_ID")
	  REFERENCES "ATLAS_PANDA"."JEDI_WORK_QUEUE" ("QUEUE_ID") ENABLE NOVALIDATE;
--------------------------------------------------------
--  Ref Constraints for Table NETWORK_MATRIX_KV
--------------------------------------------------------

  ALTER TABLE "ATLAS_PANDA"."NETWORK_MATRIX_KV" ADD CONSTRAINT "SITES_MATRIX_KV_DST_FK" FOREIGN KEY ("DST")
	  REFERENCES "ATLAS_PANDA"."SITE" ("SITE_NAME") ON DELETE CASCADE ENABLE;
  ALTER TABLE "ATLAS_PANDA"."NETWORK_MATRIX_KV" ADD CONSTRAINT "SITES_MATRIX_KV_SRC_FK" FOREIGN KEY ("SRC")
	  REFERENCES "ATLAS_PANDA"."SITE" ("SITE_NAME") ON DELETE CASCADE ENABLE;
--------------------------------------------------------
